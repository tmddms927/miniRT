...
/*   Updated: 2021/05/15 17:22:20 by seungoh          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
double	hit_cylinder(t_object *world, t_ray *ray, t_hit_record *rec)
{
	...
	rec->normal = vunit(vdivide(
		vminus(vminus(rec->p, cylinder->point),
							...
							t_ray *ray, t_hit_record *rec)
{
	...
	a = vlength2(vminus(
		ray->dir, vmult(cylinder->normal, vdot(cylinder->normal, ray->dir))));
	b = vdot(vminus(ray->dir, vmult(cylinder->normal,
			vdot(cylinder->normal, ray->dir))),
			vminus(vminus(ray->orig, cylinder->point),
			vmult(cylinder->normal,
			vdot(cylinder->normal, vminus(ray->orig, cylinder->point)))));
