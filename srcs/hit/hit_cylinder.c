...
/*   Updated: 2021/05/12 15:32:32 by seungoh          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
double		hit_cylinder(t_object *world, t_ray *ray, t_hit_record *rec)
{
	...
	rec->normal = vunit(vdivide(vminus(vminus(rec->p, cylinder->point), vmult(cylinder->normal,
					vdot(cylinder->normal, vminus(rec->p, cylinder->point)))), cylinder->radius));
	rec->albedo = world->albedo;
	if (!cylinder_height_check(cylinder, rec))
		return (FALSE);
...
t_bool			cylinder_height_check(t_cylinder *cylinder, t_hit_record *rec)
{
	...
	h = vlength2(vminus(rec->p, cylinder->point)) - cylinder->radius * cylinder->radius;
